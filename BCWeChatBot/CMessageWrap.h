/**
 * This header is generated by class-dump-z 0.2a.
 * class-dump-z is Copyright (C) 2009 by KennyTM~, licensed under GPLv3.
 *
 * Source: (null)
 */

@class VideoUploadStatInfo, iWatchAppMsgItem, MessageExpItem, MallProductItem, FavoritesItem, NSArray, MMAsset, NSData, MMWCPushInfo, EmotionPageSharedItem, WCCanvasPageItem, NSString, AppTVItem, NSMutableArray, AppInnerJumpItem, ImageInfo, CardTicketItem, EmoticonSharedItem, PushMailWrap, GiftCardItem, WCPayInfoItem, WAAppMsgItem, HardWareItem, AppProductItem, EmotionDesignerSharedItem, NSMutableDictionary;

@interface CMessageWrap : NSObject //<IAppMsgPathMgr, ISysNewXmlMsgExtendOperation, IMsgExtendOperation, NSCopying>
{
	BOOL m_bIsSplit;
	BOOL m_bNew;
	unsigned long m_uiMesLocalID;
	long long m_n64MesSvrID;
	NSString* m_nsFromUsr;
	NSString* m_nsToUsr;
	unsigned long m_uiMessageType;
	NSString* m_nsContent;
	unsigned long m_uiStatus;
	unsigned long m_uiImgStatus;
	unsigned long m_uiMsgFlag;
	unsigned long m_uiCreateTime;
	NSString* m_nsPushContent;
	NSString* m_nsMsgSource;
	NSString* m_nsRealChatUsr;
	NSData* m_dtThumbnail;
	unsigned long m_uiSendTime;
	unsigned long m_uiEmojiStatFlag;
	NSString* m_nsPattern;
	BOOL m_bForward;
	BOOL m_bCdnForward;
	unsigned long m_uiPercent;
	unsigned long m_uiDownloadStatus;
	id<IMsgExtendOperation> m_extendInfoWithMsgTypeForBiz;
	id<IMsgExtendOperation> m_extendInfoWithMsgType;
	id<IMsgExtendOperation> m_extendInfoWithFromUsr;
	NSString* m_nsLastDisplayContent;
	BOOL m_isTempSessionMsg;
	BOOL m_isEnterpriseMsg;
	unsigned long m_sequenceId;
	BOOL m_bIsFromBrandSession;
	BOOL m_bIsBrandSendMass;
	NSString* m_nsKFWorkerOpenID;
	NSString* m_nsBizClientMsgID;
	NSString* m_nsBizChatId;
	unsigned long m_uiBizChatVer;
	NSString* m_nsAtUserList;
	unsigned watchMsgSourceType;
	NSString* m_nsDisplayName;
	unsigned long _m_forwardType;
	NSMutableDictionary* _m_dicForwardParas;
}
@property(assign, nonatomic) CMessageWrap* m_refMessageWrap;
@property(readonly, copy) NSString* debugDescription;
@property(readonly, copy) NSString* description;
@property(readonly, assign) Class superclass;
@property(readonly, assign) unsigned hash;
@property(assign, nonatomic) BOOL m_bIsBrandSendMass;
@property(retain, nonatomic) NSMutableDictionary* m_dicForwardParas;
@property(assign, nonatomic) BOOL m_bIsFromBrandSession;
@property(assign, nonatomic) long m_nDownloadLimit;
@property(retain, nonatomic) NSString* m_nsMsgMd5;
@property(assign, nonatomic) BOOL m_bAppAttachExistInSvr;
@property(assign, nonatomic) unsigned long m_forwardType;
@property(retain, nonatomic) NSMutableDictionary* m_dicStatParas;
@property(retain, nonatomic) NSString* m_nsJsAppId;
@property(retain, nonatomic) NSString* m_nsShareOpenUrl;
@property(retain, nonatomic) NSString* m_nsShareOriginUrl;
@property(retain, nonatomic) NSString* m_nsMsgAttachUrl;
@property(assign, nonatomic) unsigned long m_uiApiSDKVersion;
@property(retain, nonatomic) NSString* m_nsAppID;
@property(retain, nonatomic) NSString* m_nsAppAttachID;
@property(retain, nonatomic) NSString* m_nsAppFileExt;
@property(retain, nonatomic) NSString* m_nsMsgDataUrl;
@property(assign, nonatomic) unsigned long m_uiAppDataSize;
@property(assign, nonatomic) unsigned long m_uiVoipStatus;
@property(assign, nonatomic) unsigned long m_uiAppMsgInnerType;
@property(retain, nonatomic) MessageExpItem* m_oMsgExpItem;
@property(retain, nonatomic) NSString* m_nsDisplayName;
@property(assign, nonatomic) unsigned long m_uiUploadStatus;
@property(retain, nonatomic) NSString* m_nsImgSrc;
@property(retain, nonatomic) NSArray* m_arrReaderWaps;
@property(retain, nonatomic) NSString* m_nsEmoticonMD5;
@property(retain, nonatomic) NSString* m_nsAutoDownloadControl;
@property(assign, nonatomic) unsigned long m_uiMsgThumbHeight;
@property(assign, nonatomic) unsigned long m_uiMsgThumbWidth;
@property(assign, nonatomic) unsigned long m_uiMsgThumbSize;
@property(retain, nonatomic) NSString* m_nsMsgThumbAesKey;
@property(retain, nonatomic) NSString* m_nsMsgThumbMd5;
@property(retain, nonatomic) NSString* m_nsMsgThumbUrl;
@property(retain, nonatomic) NSString* m_nsAesKey;
@property(assign, nonatomic) unsigned long m_sequenceId;
@property(assign, nonatomic) BOOL m_isEnterpriseMsg;
@property(assign, nonatomic) BOOL m_isTempSessionMsg;
@property(assign, nonatomic) unsigned watchMsgSourceType;
@property(retain, nonatomic) NSString* m_nsAtUserList;
@property(retain, nonatomic) NSString* m_nsBizClientMsgID;
@property(assign, nonatomic) unsigned long m_uiBizChatVer;
@property(retain, nonatomic) NSString* m_nsBizChatId;
@property(retain, nonatomic) NSString* m_nsKFWorkerOpenID;
@property(assign, nonatomic) unsigned long m_uiDownloadStatus;
@property(assign, nonatomic) unsigned long m_uiPercent;
@property(retain, nonatomic) NSString* m_nsPattern;
@property(assign, nonatomic) unsigned long m_uiEmojiStatFlag;
@property(assign, nonatomic) unsigned long m_uiSendTime;
@property(assign, nonatomic) BOOL m_bNew;
@property(assign, nonatomic) BOOL m_bIsSplit;
@property(retain, nonatomic) NSData* m_dtThumbnail;
@property(assign, nonatomic) BOOL m_bCdnForward;
@property(assign, nonatomic) BOOL m_bForward;
@property(retain, nonatomic) id<IMsgExtendOperation> m_extendInfoWithFromUsr;
@property(retain, nonatomic) id<IMsgExtendOperation> m_extendInfoWithMsgType;
@property(retain, nonatomic) id<IMsgExtendOperation> m_extendInfoWithMsgTypeForBiz;
@property(retain, nonatomic) NSString* m_nsRealChatUsr;
@property(retain, nonatomic) NSString* m_nsMsgSource;
@property(retain, nonatomic) NSString* m_nsPushContent;
@property(assign, nonatomic) unsigned long m_uiCreateTime;
@property(assign, nonatomic) unsigned long m_uiMsgFlag;
@property(assign, nonatomic) unsigned long m_uiImgStatus;
@property(assign, nonatomic) unsigned long m_uiStatus;
@property(retain, nonatomic) NSString* m_nsContent;
@property(assign, nonatomic) unsigned long m_uiMessageType;
@property(retain, nonatomic) NSString* m_nsToUsr;
@property(retain, nonatomic) NSString* m_nsFromUsr;
@property(assign, nonatomic) long long m_n64MesSvrID;
@property(assign, nonatomic) unsigned long m_uiMesLocalID;
@property(retain, nonatomic) NSString* m_infoUrl;
@property(retain, nonatomic) NSString* m_mapType;
@property(retain, nonatomic) NSString* m_poiName;
@property(retain, nonatomic) NSString* m_locationLabel;
@property(assign, nonatomic) double m_mapScale;
@property(assign, nonatomic) double m_longitude;
@property(assign, nonatomic) double m_latitude;
@property(retain, nonatomic) NSData* m_dtSenderMidImg;
@property(retain, nonatomic) NSString* m_AttachedContent;
@property(assign, nonatomic) float m_fLatitude;
@property(assign, nonatomic) float m_fLongitude;
@property(assign, nonatomic) unsigned long m_uiScene;
@property(assign, nonatomic) BOOL m_isForNewYear;
@property(assign, nonatomic) BOOL m_isCanUpload;
@property(retain, nonatomic) MMAsset* m_asset;
@property(retain, nonatomic) NSString* m_nsImgMidUrl;
@property(retain, nonatomic) NSString* m_nsImgHDUrl;
@property(retain, nonatomic) ImageInfo* m_oImageInfo;
@property(retain, nonatomic) NSData* m_dtImg;
@property(assign, nonatomic) unsigned long m_uiNormalImgSize;
@property(assign, nonatomic) unsigned long m_uiHDImgSize;
@property(assign, nonatomic) unsigned long m_uiEmoticonHeight;
@property(assign, nonatomic) unsigned long m_uiEmoticonWidth;
@property(assign, nonatomic) BOOL m_bShowRewardTips;
@property(retain, nonatomic) NSString* m_nsThumbImgUrl;
@property(retain, nonatomic) NSString* m_nsDesignerId;
@property(retain, nonatomic) NSString* m_aesKey;
@property(retain, nonatomic) NSString* m_encryptUrlString;
@property(retain, nonatomic) NSString* m_cdnUrlString;
@property(copy, nonatomic) NSString* m_nsEmoticonBelongToProductID;
@property(assign, nonatomic) unsigned long m_uiGameContent;
@property(assign, nonatomic) unsigned long m_uiGameType;
@property(assign, nonatomic) unsigned long m_uiEmoticonType;
@property(retain, nonatomic) VideoUploadStatInfo* m_oVideoUploadStatInfo;
@property(retain, nonatomic) NSString* m_nsStatExtStr;
@property(retain, nonatomic) NSString* m_nsStreamVideoAdUxInfo;
@property(retain, nonatomic) NSString* m_nsStreamVideoPublishId;
@property(retain, nonatomic) NSString* m_nsStreamVideoThumbUrl;
@property(retain, nonatomic) NSString* m_nsStreamVideoWebUrl;
@property(retain, nonatomic) NSString* m_nsStreamVideoWording;
@property(retain, nonatomic) NSString* m_nsStreamVideoTitle;
@property(assign, nonatomic) unsigned long m_uiStreamVideoTime;
@property(retain, nonatomic) NSString* m_nsStreamVideoUrl;
@property(retain, nonatomic) NSString* m_nsFileParam;
@property(assign, nonatomic) unsigned long m_uiVideoOffset;
@property(assign, nonatomic) unsigned long m_uiVideoSource;
@property(assign, nonatomic) unsigned long m_uiCameraType;
@property(assign, nonatomic) unsigned long m_uiVideoLen;
@property(assign, nonatomic) unsigned long m_uiVideoTime;
@property(assign, nonatomic) unsigned long m_uiVoiceCancelFlag;
@property(assign, nonatomic) unsigned long m_uiVoiceEndFlag;
@property(retain, nonatomic) NSData* m_dtVoice;
@property(assign, nonatomic) unsigned long m_uiVoiceForwardFlag;
@property(assign, nonatomic) unsigned long m_uiVoiceFormat;
@property(assign, nonatomic) unsigned long m_uiVoiceTime;
@property(assign, nonatomic) unsigned long m_duration;
@property(assign, nonatomic) unsigned long m_wordingType;
@property(assign, nonatomic) long long m_i64VoipKey;
@property(assign, nonatomic) long m_iVoipRoomid;
@property(assign, nonatomic) unsigned long m_uiVoipInviteType;
@property(assign, nonatomic) unsigned m_uiVoipRecvTime;
@property(retain, nonatomic) PushMailWrap* m_oPushMailWrap;
@property(readonly, copy) NSString* debugDescription;
@property(readonly, copy) NSString* description;
@property(readonly, assign) Class superclass;
@property(readonly, assign) unsigned hash;
@property(retain, nonatomic) NSString* m_nsInviteReason;
@property(retain, nonatomic) NSString* m_fromChatUserName;
@property(assign, nonatomic) BOOL m_bNeedReloadSubView;
@property(assign, nonatomic) BOOL m_bHasApprove;
@property(assign, nonatomic) BOOL m_bNewApprove;
@property(assign, nonatomic) BOOL m_bNewInv;
@property(retain, nonatomic) NSString* m_nsInviteTickets;
@property(retain, nonatomic) NSMutableArray* m_arrInviteeMembers;
@property(retain, nonatomic) NSString* m_inviterName;
@property(assign, nonatomic) int m_msgInnerType;
@property(retain, nonatomic) NSString* m_newXmlType;
@property(retain, nonatomic) NSString* m_sessionShowContent;
@property(retain, nonatomic) NSString* m_url;
@property(retain, nonatomic) NSString* m_qrCode;
@property(retain, nonatomic) NSMutableArray* m_arrMembers;
@property(retain, nonatomic) NSString* m_scene;
@property(retain, nonatomic) NSString* m_showContent;
@property(readonly, copy) NSString* debugDescription;
@property(readonly, copy) NSString* description;
@property(readonly, assign) Class superclass;
@property(readonly, assign) unsigned hash;
@property(assign, nonatomic) unsigned long m_uiWeAppVersion;
@property(assign, nonatomic) unsigned long m_uiWeAppState;
@property(retain, nonatomic) WAAppMsgItem* m_oWAAppItem;
@property(assign, nonatomic) unsigned long m_uiDownloadPercent;
@property(retain, nonatomic) NSString* m_nsStatExtStr;
@property(retain, nonatomic) NSString* m_nsStreamVideoAdUxInfo;
@property(retain, nonatomic) NSString* m_nsStreamVideoPublishId;
@property(retain, nonatomic) NSString* m_nsStreamVideoThumbUrl;
@property(retain, nonatomic) NSString* m_nsStreamVideoWebUrl;
@property(retain, nonatomic) NSString* m_nsStreamVideoWording;
@property(retain, nonatomic) NSString* m_nsStreamVideoTitle;
@property(assign, nonatomic) unsigned long m_uiStreamVideoTime;
@property(retain, nonatomic) NSString* m_nsStreamVideoUrl;
@property(assign, nonatomic) unsigned long m_uiAppContentAttributeBitSetFlag;
@property(assign, nonatomic) BOOL m_isReaderForbidForward;
@property(readonly, assign, nonatomic) BOOL m_isContentOriginal;
@property(retain, nonatomic) NSString* m_nsTemplateId;
@property(assign, nonatomic) BOOL m_isDirectSend;
@property(retain, nonatomic) WCCanvasPageItem* m_oCanvasPageItem;
@property(assign, nonatomic) BOOL m_bIsForceUpdate;
@property(retain, nonatomic) NSString* m_nsAppMessageAction;
@property(retain, nonatomic) NSString* m_nsAppMessageExt;
@property(retain, nonatomic) MMWCPushInfo* m_oWebcachePushInfo;
@property(retain, nonatomic) iWatchAppMsgItem* m_oiWatchMsgItem;
@property(retain, nonatomic) AppInnerJumpItem* m_oAppInnerJumpItem;
@property(retain, nonatomic) WCPayInfoItem* m_oWCPayInfoItem;
@property(retain, nonatomic) HardWareItem* m_oHardWareItem;
@property(retain, nonatomic) FavoritesItem* m_oRecordItem;
@property(retain, nonatomic) AppTVItem* m_oAppTVItem;
@property(retain, nonatomic) EmotionPageSharedItem* m_oEmotionPageSharedItem;
@property(retain, nonatomic) EmotionDesignerSharedItem* m_oEmotionDesignerSharedItem;
@property(retain, nonatomic) EmoticonSharedItem* m_oEmoticonSharedItem;
@property(retain, nonatomic) GiftCardItem* m_oGiftCardItem;
@property(retain, nonatomic) CardTicketItem* m_oCardTicketItem;
@property(retain, nonatomic) AppProductItem* m_oAppProductItem;
@property(retain, nonatomic) MallProductItem* m_oMallProductItem;
@property(retain, nonatomic) NSString* m_nsAppMediaTagName;
@property(assign, nonatomic) unsigned long m_uiEncryVer;
@property(retain, nonatomic) NSArray* m_arrCustomWrap;
@property(retain, nonatomic) NSString* m_nsAppName;
@property(retain, nonatomic) NSString* m_nsThumbUrl;
@property(assign, nonatomic) unsigned long m_uiOriginMsgSvrId;
@property(assign, nonatomic) unsigned long m_uiOriginFormat;
@property(assign, nonatomic) unsigned long m_uiRemindFormat;
@property(assign, nonatomic) unsigned long m_uiRemindAttachTotalLen;
@property(retain, nonatomic) NSString* m_nsRemindAttachId;
@property(assign, nonatomic) unsigned long m_uiRemindId;
@property(assign, nonatomic) unsigned long m_uiRemindTime;
@property(retain, nonatomic) NSString* m_nsSourceDisplayname;
@property(retain, nonatomic) NSString* m_nsSourceUsername;
@property(retain, nonatomic) NSString* m_nsAppExtInfo;
@property(assign, nonatomic) unsigned long m_uiAppExtShowType;
@property(assign, nonatomic) unsigned long m_uiAppVersion;
@property(retain, nonatomic) NSString* m_nsAppMediaLowBandDataUrl;
@property(retain, nonatomic) NSString* m_nsAppMediaDataUrl;
@property(retain, nonatomic) NSString* m_nsAppMediaLowUrl;
@property(retain, nonatomic) NSString* m_nsAppMediaUrl;
@property(retain, nonatomic) NSString* m_nsAppContent;
@property(assign, nonatomic) unsigned long m_uiSoundType;
@property(assign, nonatomic) unsigned long m_uiShowType;
@property(retain, nonatomic) NSString* m_nsAppAction;
@property(retain, nonatomic) NSString* m_nsDesc;
@property(retain, nonatomic) NSString* m_nsTitle;
+(id)createMaskedThumbImageForMessageWrap:(id)messageWrap;
+(id)GetCdnDownloadPathOfMsgThumb:(id)msgThumb;
+(id)GetTempPathOfMesShortVideoWithMessageWrap:(id)messageWrap;
+(id)GetPathOfMesVideoWithMessageWrap:(id)messageWrap;
+(id)getMaskedVideoMsgImgThumb:(id)thumb;
+(id)getMaskedMsgImgThumb:(id)thumb;
+(id)getMsgImgThumb:(id)thumb;
+(id)getPathOfVideoMsgImgThumb:(id)videoMsgImgThumb;
+(id)GetPathOfMaskedSquareMesImgThumbDir;
+(id)GetPathOfMaskedSquareMesImgThumbDir:(id)maskedSquareMesImgThumbDir;
+(id)GetPathOfSquareMesImgThumb:(id)squareMesImgThumb;
+(id)getPathOfMaskedVideoMsgImgThumb:(id)maskedVideoMsgImgThumb;
+(id)getPathOfMaskedMsgImgThumb:(id)maskedMsgImgThumb;
+(id)getPathOfMessageImageCache;
+(id)getOldPathOfMessageImageCache;
+(id)getPathOfMiddleImgForSender:(id)sender;
+(id)getPathOfMsgImgThumb:(id)msgImgThumb;
+(id)getMsgMiddleImgData:(id)data;
+(id)getMsgMiddleImg:(id)img;
+(id)getMsgImgData:(id)data;
+(id)getMsgImg:(id)img;
+(id)getPathOfMsgImg:(id)msgImg;
+(id)getMsgHDImgData:(id)data;
+(id)getJpgPathOfMsgImg:(id)msgImg;
+(BOOL)isVcodec2Img:(id)img;
+(BOOL)MsgImgExist:(id)exist;
+(id)getJpgPathOfMsgHDImg:(id)msgHDImg;
+(BOOL)MsgHDImgExist:(id)exist;
+(id)getMsgHDImg:(id)img;
+(id)getPathOfMsgVcodec2HDImg:(id)msgVcodec2HDImg;
+(id)getPathOfMsgVcodec2Img:(id)msgVcodec2Img;
+(id)getUserNameFromMsgWrap:(id)msgWrap;
+(BOOL)isSenderFromMsgWrap:(id)msgWrap;
+(BOOL)IsRecordMsg:(id)msg;
+(BOOL)SaveMesImg:(id)img MsgWrap:(id)wrap;
+(BOOL)SaveMsgThumbWithMsgWrap:(id)msgWrap;
+(void)clearLocalMaskedThumbImage:(id)image;
+(void)clearLocalImage:(id)image;
+(id)FormMessageWrapFromAddMsg:(id)addMsg;
+(id)FormMessageWrapFromBuffer:(id)buffer;
+(void)initialize;
+(void)GetPathOfAppRemindAttach:(id)appRemindAttach retStrPath:(id*)path;
+(void)GetPathOfAppThumb:(id)appThumb LocalID:(unsigned long)anId retStrPath:(id*)path;
+(void)GetPathOfMaskedAppThumb:(id)maskedAppThumb LocalID:(unsigned long)anId retStrPath:(id*)path;
+(void)GetPathOfAppDataTemp:(id)appDataTemp LocalID:(unsigned long)anId retStrPath:(id*)path;
+(void)GetPathOfAppDataTemp:(id)appDataTemp LocalID:(unsigned long)anId AttachId:(id)anId3 retStrPath:(id*)path;
+(void)GetPathOfAppDataByUserName:(id)appDataByUserName andMessageWrap:(id)wrap retStrPath:(id*)path;
+(void)GetPathOfAppDataByUserName:(id)appDataByUserName andMessageWrap:(id)wrap andAttachId:(id)anId andAttachFileExt:(id)ext retStrPath:(id*)path;
+(void)GetPathOfAppData:(id)appData LocalID:(unsigned long)anId FileExt:(id)ext retStrPath:(id*)path;
+(void)GetPathOfAppImgCacheDir:(id)appImgCacheDir retStrPath:(id*)path;
+(void)GetPathOfAppDir:(id)appDir retStrPath:(id*)path;
+(id)getMessageListStatusImage:(unsigned long)image;

-(BOOL)isSentOK;
-(BOOL)IsNewInvitationApprove;
-(BOOL)IsNewInvitation;
-(BOOL)IsRoomAnnouncement;
-(BOOL)IsAtMe;
-(BOOL)isShowAppMessageBlockButton;
-(BOOL)isShowAppBottomButton;
-(id)keyDescription;
-(id)getMsgExtBizId;
-(BOOL)IsNeedAddMsgBizExt;
-(BOOL)IsNeedChatExt;
-(void)genMiddleImg:(id)img originImgData:(id)data isSaveOK:(BOOL)ok delegate:(id)delegate;
-(BOOL)genImageFromMMAssetAndNotify:(id)mmassetAndNotify;
-(id)GetDisplayContent;
-(id)GetThumb;
-(id)GetMsgClientMsgID;
-(BOOL)IsSameMsgWithFullCheck:(id)fullCheck;
-(BOOL)IsSameMsg:(id)msg;
-(BOOL)IsSendBySendMsg;
-(BOOL)IsAppMessage;
-(BOOL)IsPureVideoMsg;
-(BOOL)IsShortMovieMsg;
-(BOOL)IsVideoMsg;
-(BOOL)IsSimpleVideoMsg;
-(BOOL)IsImgMsg;
-(BOOL)IsTextMsg;
-(BOOL)IsChatRoomMessage;
-(BOOL)IsMassSendMessage;
-(BOOL)IsBottleMessage;
-(BOOL)IsQQMessage;
-(BOOL)IsSxMessage;
-(id)GetChatName;
-(void)AddTagToMsgSource:(id)msgSource value:(id)value;
-(void)UpdateMsgSource;
-(void)ChangeForDisplay;
-(void)ChangeForBackup;
-(void)fillMsgSourceFromContact:(id)contact isFromTempSession:(BOOL)tempSession;
-(void)ChangeForMsgSource;
-(void)ChangeForChatRoom;
-(id)forwardingTargetForSelector:(SEL)selector;
-(void)forwardInvocation:(id)invocation;
-(id)methodSignatureForSelector:(SEL)selector;
-(id)copyWithZone:(NSZone*)zone;
-(id)initWithMsgType:(int)msgType nsFromUsr:(id)usr;
-(id):(int)msgType;
-(id)init;
-(id)wishingString;
-(BOOL)bIsAppUrlTypeWithCanvas;
-(id)nativeUrl;
-(int)yoType;
-(unsigned)yoCount;
-(id)getNodeBtnList;
-(int)compareQQAscending:(id)ascending;
-(int)compareSXAscending:(id)ascending;
-(int)compareMessageAscending:(id)ascending;
-(int)compareMessageLocalIDDescending:(id)descending;
-(BOOL)canPlayVideo:(id)video;
-(BOOL)isAd;
-(id)GetCdnDownloadThumbPathOfVideo;
-(id)GetCdnDownloadPathOfVideo;
@end

